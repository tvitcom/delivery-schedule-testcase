...
class RegionController extends Controller
{
    ...
    /**
     * Search records Region with default search string.
     */
    public function actionIndex()
    {
        $model = new RegionSearchForm;
        $criteria = new CDbCriteria;

        if (isset($_POST['RegionSearchForm'])) {
            $model->attributes = $_POST['RegionSearchForm'];

            if ($model->validate()) {
                $criteria->compare('name', $model->search_string, true);
            }
        } else {
            //Default search string 'моск'
            $criteria->condition = 'id < 40';
        }
